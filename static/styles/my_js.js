...
//Load json
fetch("static/resource/json/data.json")
  .then((response) => {
    return response.json();
  })
  .then((data) => {
...
const search_input_for_design_id = document.getElementById(
  "search_input_for_design_id"
);
...
function update_form() {
  ...
  if (form_num == 2) {
    ...
    if (submit_dict.image_src) {
      ...
      const check_click_circle = (x, y) => {
        return Math.abs(x - y) < radius;
      };
      const draw_circle = (x, y, radius, color = { R: 243, G: 1, B: 1 }) => {
        draw_context.fillStyle = `rgb(${color.R},${color.G},${color.B})`;
        draw_context.beginPath();
        draw_context.arc(x, y, radius, 0, 2 * Math.PI);
        draw_context.fill();
        draw_context.closePath();
      };
      const draw_handles = (color) => {
        draw_circle(shape["X"], shape["Y"], radius, color);
        draw_circle(shape["X"] + shape["WIDTH"], shape["Y"], radius, color);
        draw_circle(
          shape["X"] + shape["WIDTH"],
        ...
        draw_circle(shape["X"], shape["Y"] + shape["HEIGHT"], radius, color);
      };

      const drawRectangle = (shape, color = { R: 243, G: 1, B: 1, A: 0.3 }) => {
        draw_handles(color);
        draw_context.beginPath();
        draw_context.strokeStyle = `rgb(${color.R},${color.G},${color.B})`;
        draw_context.lineWidth = 4; // increase the border width
        draw_context.fillStyle = `rgb(${color.R},${color.G},${color.B},${color.A})`; // add background color to shape
        draw_context.roundRect(
          shape["X"],
          shape["Y"],
          shape["WIDTH"],
          shape["HEIGHT"],
      ...
      const mouseDownListener = (event) => {
        ...
        if (!is_accepted) {
          ...
          shape["X"] = startPosition.x;
          shape["Y"] = startPosition.y;
        ...
        //For Drag Context
          ...
          if (draw_context.isPointInPath(startPosition.x, startPosition.y)) {
            is_down = true;
          }
          if (
      ...
      const mouseMoveListener = (event) => {
        ...
        if (!is_accepted) {
          ...
          shape["X1"] = endPosition.x;
          shape["Y1"] = endPosition.y;
        ...
        //For Drag Context
          ...
          if (!is_down) {
            ...
            endPosition = getClientOffset(event);
            if (dragTL) {
              shape["WIDTH"] += shape["X"] - endPosition.x;
              shape["HEIGHT"] += shape["Y"] - endPosition.y;

              shape["X"] = endPosition.x;
              shape["Y"] = endPosition.y;
            } else if (dragTR) {
              shape["WIDTH"] = Math.abs(shape["X"] - endPosition.x);
              shape["HEIGHT"] += shape["Y"] - endPosition.y;
              shape["Y"] = endPosition.y;
            } else if (dragBL) {
              shape["WIDTH"] += shape["X"] - endPosition.x;
              shape["HEIGHT"] = Math.abs(shape["Y"] - endPosition.y);
              shape["X"] = endPosition.x;
            } else if (dragBR) {
              shape["WIDTH"] = Math.abs(shape["X"] - endPosition.x);
              shape["HEIGHT"] = Math.abs(shape["Y"] - endPosition.y);
      ...
      const accepted = (event) => {
        ...
        console.log(shape);
        // submit_dict.feature_position = [shape['X'], shape['Y'],shape['X1'], shape['Y1']];
  ...
  if (form_num == 3) {
    $("#prev_class_id").html(submit_dict.class_id || "None");
    $("#prev_design_id").html(submit_dict.design_id || "None");
    $("#prev_image_src").attr("src", submit_dict.image_src || none_image);
    $("#prev_feature_image_src").attr(
      "src",
      submit_dict.feature_im || none_image
    );
    $("#prev_injection_image_src").attr(
      "src",
      submit_dict.rejection_im || none_image
    );
  }
}
